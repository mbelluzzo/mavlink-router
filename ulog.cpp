...
bool ULog::start()
{
    ...
    snprintf(path, sizeof(path), "%s/%i-%02i-%02i_%02i-%02i-%02i.ulg", _logs_dir,
             timeinfo->tm_year + 1900, timeinfo->tm_mon + 1, timeinfo->tm_mday, timeinfo->tm_hour,
             timeinfo->tm_min, timeinfo->tm_sec);
    _file = open(path, O_WRONLY | O_CLOEXEC | O_CREAT | O_NONBLOCK | O_TRUNC,
    ...
    log_info("Logging target system_id=%u on %s", _target_system_id, path);
    return true;
}
...
bool ULog::_logging_seq(uint16_t seq, bool *drop)
{
    ...
    if (_expected_seq != seq) {
        *drop = true;
        ...
        /*
         * Handle case were Flight Stack did not received a ACK and sent the
         * same message again.
         */
        if (_expected_seq - 1 == seq)
            return false;
...
void ULog::_send_msg(const mavlink_message_t *msg, int target_sysid)
{
    ...
    _mainloop->route_msg(msg, target_sysid);
}
